...
create_sum_data <- function(title,
  ...
  colnames(data_sum) <- c(split_by,count_by, "ymax","ymin","label","ylabelPosition")
...
plot_sum_data <- function(title, split_by, count_by , data_sum) {
  ...
  ggplot(data_sum,
    ...
    geom_rect() +
    geom_text_repel(size = 8, x = 1, aes(y= ylabelPosition, label = data_sum[,count_by], color = data_sum[,split_by]),
