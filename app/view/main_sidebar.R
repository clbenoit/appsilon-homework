...
box::use(
  ...
  shiny[h3, moduleServer, NS, tagList, fluidRow, column, req, observe, reactive, 
        reactiveVal, observeEvent,  bindCache, 
        selectizeInput, updateSelectizeInput, reactiveValues, bindEvent, checkboxGroupInput, icon,
        dateRangeInput, updateDateRangeInput, br],
  dplyr[`%>%`, filter]
)
...
ui <- function(id) {
  ...
  tagList(
    fluidRow(
      selectizeInput(inputId = ns("lifeStage"), label = "lifeStage", multiple = TRUE, selected = NULL, choices = NULL),br(),
      checkboxGroupInput(ns("sex"), "sex", width = "100%", inline = TRUE,
                         choiceNames =
                           list(icon("mars"), icon("venus"),
                                icon("question")),
                         choiceValues =
                           list("male","female","undetermined"),
                         selected = c("male","female","undetermined")),br(),
      dateRangeInput(ns("dateRange"), "Date Range", start = as.Date("2004-01-01", "%Y-%m-%d"), end = Sys.Date(), format = "yyyy-mm-dd"),
      selectizeInput(inputId = ns("continent"), label = "Continent", multiple = TRUE, selected = NULL, choices = NULL)
      )
    )
}
